...
class WSGIFileServerApplication(object):
    ...
    def __init__(self, root_path, mount_point=None, index_file = "index.html" ):
    ...
    def handler(self, environ, start_response):
        ...
        serve_file = serve_file.replace("/", "", 1)
        if(serve_file==""):
            serve_file = self.index_file
        ...
        def do_get():
            ...
            try:
                ...
                start_response('404 Not found', [('Content-Type', 'text/plain')])
                return ['404 Not Found ' + os.path.join(self.path, serve_file)]
            response = FileResponse(f, os.path.join(self.path, serve_file))
            time_format = "%a, %d %b %Y %H:%M:%S %Z"
            start_response('200 OK', [#('Cache-Control','no-cache'), ('Pragma','no-cache'),
                                      ('Content-Length', str(response.size),),
                                      ('Expires',(datetime.datetime.now()+ datetime.timedelta(+10)).strftime(time_format)),
